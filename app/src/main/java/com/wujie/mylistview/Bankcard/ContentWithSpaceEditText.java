    ...
    private TextWatcher watcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (isMiddle || isNeedSpace) {
                ...
                Log.d("wujie", "onTextChanged: "+newStr+"是否隐藏："+newStr);
    ...
    public boolean checkTextRight(){
        ...
        if (contentType == TYPE_PHONE) {
            if (TextUtils.isEmpty(text)) {
                Log.d("wujie","手机号不能为空，请输入正确的手机号");

            } else if (text.length() < 11) {
                Log.d("wujie","手机号不足11位，请输入正确的手机号");
...
//                ToastUtil.show(getContext(), "身份证号不能为空，请输入正确的身份证号");
                Log.d("wujie","身份证号不能为空，请输入正确的身份证号");
            } else if (text.length() < 18) {
                Log.d("wujie","身份证号不正确，请输入正确的身份证号");
            } else {
                return true;
            }
        }
